...
(() => {
  ...
  const isAdmin=()=>localStorage.getItem('vortexus_admin_session')==='1';
  ...
  const adminLogout=()=>{localStorage.removeItem('vortexus_admin_session');location.href='admin-login.html'};
  ...
  async function initSite(){
    ...
    $('#uploadForm')?.addEventListener('submit',async e=>{
      ...
      if(!session){
        err.textContent='🔐 Kamu harus login terlebih dahulu sebelum mengunggah foto atau video.';
        err.hidden=false;
        setTimeout(()=>{location.href='login.html?next=upload'},700);
        return;
      }
  ...
  function initAdminLogin(){
    ...
    form.addEventListener('submit',e=>{
      e.preventDefault();
      const u=$('#adminUsername').value.trim(), p=$('#adminPassword').value, err=$('#adminLoginError');
      err.hidden=true;
      if(u==='VORTEXUSADMIN' && p==='ADMINVORTEX26'){
        localStorage.setItem('vortexus_admin_session','1');
        location.href='admin.html';
      }else{
        err.textContent='Username atau password admin salah.'; err.hidden=false;
      }
    });
  }
